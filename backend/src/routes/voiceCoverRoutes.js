...
router.post("/voices",upload.array("samples",10),async(req,res)=>{
  ...
  if(!process.env.VOICE_PROFILE_PROVIDER_URL||!process.env.VOICE_PROFILE_PROVIDER_KEY) return fail(res,501,"Voice profile service is not configured yet. Add an authorized custom-voice provider to Render environment variables.","VOICE_PROVIDER_NOT_CONFIGURED");
  return fail(res,501,"Voice provider credentials are present, but its provider adapter still needs to be connected using the provider's documented API.","VOICE_PROVIDER_ADAPTER_REQUIRED");
});
